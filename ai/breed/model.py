...
class DogBreedClassifier:
    def __init__(self):
        try:
            ...
            self.model = resnet50()
            # 로컬 가중치 로드
            state_dict = torch.load("/root/.cache/torch/hub/checkpoints/resnet50-11ad3fa6.pth", weights_only=True)
            self.model.load_state_dict(state_dict)
            self.model.eval()
            self.model.to(self.device)
            logger.info("ResNet50 model loaded successfully with local weights")
