...
export default function DogResearchLabPage() {
  ...
  const [moodInputType, setMoodInputType] = useState<"photo" | "video">(
    "photo",
  );
  ...
  const getBreedInfo = (breed: string) => {
    const breedInfoMap: Record<
      string,
      { characteristics: string[]; description: string }
    > = {
      "골든 리트리버": {
        characteristics: ["온순함", "활발함", "지능적", "가족친화적"],
        description:
          "골든 리트리버는 매우 온순하고 지능적인 견종으로, 가족과 함께하는 것을 좋아합니다.",
      },
      "래브라도 리트리버": {
        characteristics: ["충성심", "에너지 넘침", "훈련 잘됨", "수영 좋아함"],
        description:
          "래브라도 리트리버는 충성심이 강하고 에너지가 넘치는 견종입니다.",
      },
      치와와: {
        characteristics: ["작은 체구", "용감함", "활발함", "주인에게 충성"],
        description:
          "치와와는 세계에서 가장 작은 견종 중 하나로, 작지만 용감한 성격을 가지고 있습니다.",
      },
      믹스견: {
  ...
  const predictBreeding = async () => {
    ...
    try {
      ...
      const response = await axios.post("/api/ai/predict-breeding", formData);
      const result = response.data;
    ...
    setTimeout(() => {
      const mock: MoodAnalysisResult[] = [
        ...
        },
        {
          ...
          description:
            "강아지가 약간 불안해하는 것 같습니다. 귀가 뒤로 젖혀져 있고 몸을 움츠리고 있어요.",
        },
        {
          mood: "평온함",
          confidence: 85,
          ...
          description:
            "강아지가 매우 평온하고 안정된 상태입니다. 편안하게 쉬고 있거나 만족스러워하고 있어요.",
        },
      ];
      setMoodResult(mock[Math.floor(Math.random() * mock.length)]);
      ...
      <div className="container mx-auto px-4 py-8">
        ...
        <div className="flex justify-center mb-8">
          <div className="flex bg-white rounded-lg p-1 shadow-md">
            {(["identify", "breeding", "mood"] as const).map((t) => (
              <button
                key={t}
                  ...
                  <div className="border-2 border-dashed border-gray-300 rounded-lg p-8 hover:border-blue-400 transition-colors">
                    {uploadedFile ? (
                      <div className="space-y-4">
                        ...
                        <Button
                          onClick={() =>
                            document.getElementById("breed-image-upload")?.click()
                          }
                          variant="outline"
                        >
                          <Camera className="w-4 h-4 mr-2" />
                    ...
                    <CardContent className="p-6">
                      <div className="text-center mb-4">
                        ...
                        <div className="text-3xl font-bold text-blue-600">
                          {identificationResult.breed}
                        </div>
                        <div className="text-lg text-gray-600">
                          정확도: {identificationResult.confidence}%
                        </div>
                      </div>
                      ...
                      <div className="space-y-4">
                        <div>
                          ...
                          <div className="flex flex-wrap gap-2">
                            {identificationResult.characteristics.map(
                              (trait, index) => (
                                <Badge
                                  key={index}
            ...
            <Card className="bg-yellow-50 border-yellow-200">
              <CardContent className="p-4">
                <p className="text-sm text-yellow-800 text-center">
                  ⚠️ 이 예측은 AI 시뮬레이션 결과이며, 실제 교배 결과와 다를 수 있습니다. 반려동물의 교배는
                  전문가와 상담 후 신중하게 결정해주세요.
                </p>
              </CardContent>
            </Card>
          </div>
          ...
          <div className="max-w-4xl mx-auto">
            <Card className="mb-8">
              <CardHeader>
                ...
                <p className="text-center text-gray-600">
                  사진이나 동영상을 통해 강아지의 기분 상태를 분석해드려요!
                </p>
              </CardHeader>
              <CardContent className="space-y-6">
                    ...
                    {moodInputType === "photo" ? (
                      moodImage ? (
                        <div className="space-y-4">
                          ...
                          <Button
                            onClick={() =>
                              document.getElementById("mood-image-upload")?.click()
                            }
                            variant="outline"
                          >
                            <Camera className="w-4 h-4 mr-2" />
                      ...
                      <div className="space-y-4">
                        <video
                          src={moodVideo}
                          controls
                          className="mx-auto rounded-lg max-w-full h-64 object-cover"
                        />
                        <Button
                          onClick={() =>
                            document.getElementById("mood-video-upload")?.click()
                          }
                          variant="outline"
                        >
                          <Video className="w-4 h-4 mr-2" />
                        ...
                        <div className="space-y-6">
                          <div>
                            ...
                            <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
                              {Object.entries(moodResult.emotions).map(
                                ([emotion, value]) => (
